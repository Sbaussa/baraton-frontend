  ...
  return (
    <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-4 flex flex-col gap-3 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between">
        {/* Icono más grande: w-11 h-11 en lugar de w-8 h-8 */}
      ...
      <div>
        ...
        {/* Valor más grande: text-xl en lugar de text-lg */}
        <p className={`text-xl font-bold tracking-tight leading-tight ${valueColor}`}>{value}</p>
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    // Un poco más grande: text-xs → text-sm, más margen
    <h3 className="text-sm font-semibold text-stone-500 uppercase tracking-wider mb-4">{children}</h3>
  );
}
...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-stone-50 p-3 sm:p-4 md:p-6 space-y-4">

      {/* Header */}
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-stone-800 tracking-tight">Dashboard</h1>
          <p className="text-sm text-stone-400 mt-0.5 flex items-center gap-1.5 flex-wrap">
            {isToday
            ...
            {lastUpdated && (
              <span className="text-xs text-stone-300 flex items-center gap-1">
                · <RefreshCw size={11} /> {lastUpdated.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' })}
              </span>
            )}
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {mode === 'day' ? (
          <div className="flex items-center gap-2">
            <input type="date"
              className="bg-white border border-stone-200 rounded-xl px-3 py-2 text-sm text-stone-700 focus:outline-none focus:ring-2 focus:ring-orange-300"
              value={date} max={TODAY} onChange={(e) => setDate(e.target.value)} />
            {date !== TODAY && (
              <button onClick={() => setDate(TODAY)} className="text-sm text-orange-500 hover:text-orange-400 font-medium">Hoy</button>
            )}
          </div>
        ) : (
      ...
      {loading ? (
        ...
        <>
          {/* KPIs principales — icono size 20 */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <StatCard icon={<DollarSign size={20} />} label="Ingresos totales"
            ...
            <StatCard icon={<ShoppingBag size={20} />} label="Pedidos entregados"
              value={stats?.totalOrders || 0}
              sub={`${conversionRate}% tasa de éxito`}
              iconBg="bg-orange-50" iconColor="text-orange-500" />
            <StatCard icon={<Target size={20} />} label="Ticket promedio"
              value={`$${Math.round(stats?.avgTicket || 0).toLocaleString('es-CO')}`}
              iconBg="bg-blue-50" iconColor="text-blue-500" />
            <StatCard icon={<Clock size={20} />} label="Pendientes ahora"
          ...
          {/* KPIs secundarios */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <StatCard icon={<XCircle size={20} />} label="Cancelados"
              value={stats?.cancelledOrders || 0}
              iconBg="bg-red-50" iconColor="text-red-400" />
            <StatCard icon={<Award size={20} />} label="Producto estrella"
            ...
            <StatCard icon={<Tag size={20} />} label="Categoría top"
            ...
            <StatCard icon={<CreditCard size={20} />} label="Pago más usado"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-4">
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-semibold text-stone-500 uppercase tracking-wider">Conversión</p>
                <Zap size={16} className="text-orange-400" />
              </div>
              <div className="flex items-end gap-2 mb-3">
                <span className="text-3xl font-bold text-stone-800 tracking-tight">{conversionRate}%</span>
                <span className="text-sm text-stone-400 mb-1">de éxito</span>
              </div>
              <div className="h-2.5 bg-stone-100 rounded-full overflow-hidden">
                <div className="h-2.5 bg-gradient-to-r from-orange-400 to-emerald-400 rounded-full transition-all duration-700"
            ...
            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-4">
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-semibold text-stone-500 uppercase tracking-wider">Estado</p>
                <Package size={16} className="text-stone-300" />
              </div>
              <div className="space-y-2.5">
                {(statusBreakdown.length > 0 ? statusBreakdown : ['PENDING','PREPARING','READY','DELIVERED','CANCELLED'].map(st => ({ status: st, _count: 0 }))).map((s: any) => (
                  <div key={s.status} className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                      <div className={`w-2.5 h-2.5 rounded-full ${STATUS_DOTS[s.status] || 'bg-stone-300'}`} />
                      <span className="text-stone-600">{STATUS_LABELS[s.status] || s.status}</span>
                    </div>
                    <span className={`font-semibold ${s._count > 0 ? 'text-stone-700' : 'text-stone-300'}`}>{s._count}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-4">
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-semibold text-stone-500 uppercase tracking-wider">Pagos</p>
              ...
              {paymentData.length === 0 ? (
                ...
                <div className="space-y-3">
                  {paymentData.map((p: any) => {
                    const pct = stats?.totalRevenue > 0 ? Math.round((p.value / stats.totalRevenue) * 100) : 0;
                    return (
                      <div key={p.name}>
            ...
            <div className="lg:col-span-2 bg-white rounded-2xl border border-stone-100 shadow-sm p-4 md:p-5">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={byHour} barGap={4}>
                  ...
                  <XAxis dataKey="hour" tick={{ fill: '#C4B8B0', fontSize: 12 }} tickFormatter={(h) => `${h}h`} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fill: '#C4B8B0', fontSize: 12 }} width={48} axisLine={false} tickLine={false} />
              ...
              <div className="flex items-center gap-4 mt-2">
                <div className="flex items-center gap-2 text-sm text-stone-400">
                  <div className="w-3 h-3 rounded bg-orange-400" /> Ingresos
                </div>
                <div className="flex items-center gap-2 text-sm text-stone-400">
                  <div className="w-3 h-3 rounded bg-stone-200" /> Pedidos
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-4 md:p-5">
              ...
              {paymentData.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-stone-300">
                  <CreditCard size={32} />
                  <p className="text-sm mt-2">Sin pagos registrados</p>
                </div>
              ) : (
                <>
                  <ResponsiveContainer width="100%" height={160}>
                    <PieChart>
                      <Pie data={paymentData} cx="50%" cy="50%" innerRadius={46} outerRadius={72}
                    ...
                    {paymentData.map((p: any) => (
                      <div key={p.name} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-stone-500">{p.name}</span>
                        </div>
                        <div className="text-right">
                          <span className="text-sm font-semibold text-stone-700">${p.value.toLocaleString('es-CO')}</span>
              ...
              <div className="space-y-4">
                {stats?.topProducts?.length ? stats.topProducts.slice(0, 6).map((p: any, i: number) => (
                  <div key={p.id} className="flex items-center gap-3">
                    <span className={`text-sm font-bold w-6 text-center flex-shrink-0 ${i === 0 ? 'text-orange-500' : 'text-stone-300'}`}>
                      {i === 0 ? '🥇' : i + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <p className="text-sm text-stone-700 font-medium truncate pr-2">{p.name}</p>
                        <span className="text-sm text-stone-500 flex-shrink-0 font-semibold">{p.totalSold} uds</span>
                      </div>
                      <div className="h-2 bg-stone-100 rounded-full overflow-hidden">
                        <div className="h-2 bg-gradient-to-r from-orange-400 to-orange-300 rounded-full transition-all duration-500"
                  ...
                  <div className="flex flex-col items-center py-8 text-stone-300">
                    <Package size={28} />
                    <p className="text-sm mt-2">Sin ventas registradas</p>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="space-y-4">
                {stats?.categoryRanking?.length ? stats.categoryRanking.slice(0, 6).map((cat: any, i: number) => (
                  <div key={cat.name} className="flex items-center gap-3">
                    <span className={`text-sm font-bold w-6 text-center flex-shrink-0 ${i === 0 ? 'text-blue-500' : 'text-stone-300'}`}>
                      {i === 0 ? '⭐' : i + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <p className="text-sm text-stone-700 font-medium truncate pr-2">{cat.name}</p>
                        <span className="text-sm text-stone-500 flex-shrink-0 font-semibold">{cat.total} uds</span>
                      </div>
                      <div className="h-2 bg-stone-100 rounded-full overflow-hidden">
                        <div className="h-2 bg-gradient-to-r from-blue-400 to-blue-300 rounded-full transition-all duration-500"
            ...
            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-4 md:p-5">
              ...
              <div className="space-y-3.5">
                {stats?.recentOrders?.length ? stats.recentOrders.map((order: any) => (
                  <div key={order.id} className="flex items-start gap-3 pb-3.5 border-b border-stone-50 last:border-0 last:pb-0">
                    <div className={`w-2.5 h-2.5 rounded-full mt-1.5 flex-shrink-0 ${STATUS_DOTS[order.status] || 'bg-stone-300'}`} />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-stone-700 text-sm font-bold">#{order.id}</span>
                  ...
                  <div className="flex flex-col items-center py-8 text-stone-300">
                    <ShoppingBag size={28} />
                    <p className="text-sm mt-2">Sin pedidos recientes</p>
                  </div>
                )}
              </div>
            </div>
