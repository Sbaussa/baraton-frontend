...
function PaymentModal({ order, onClose, onDone }: { order: Order; onClose: () => void; onDone: () => void }) {
  ...
  const confirm = async () => {
    ...
    setSaving(true);
    try {
      await ordersApi.processPayment(order.id, { paymentMethod: method, cashGiven: method === 'EFECTIVO' ? Number(cashGiven) : undefined, markDelivered: false });
      onDone(); onClose();
    ...
    <Modal open title={`Cobrar — ${order.orderNumber}`} onClose={onClose} size="sm">
      <div className="space-y-4">
        <div className="text-center">
          ...
          {order.delivery?.customerName && <p className="text-sm text-stone-500 mt-1">👤 {order.delivery.customerName}</p>}
        </div>
        <div className="grid grid-cols-3 gap-2">
          {[{k:'EFECTIVO',i:'💵',l:'Efectivo'},{k:'TRANSFERENCIA',i:'📲',l:'Transferencia'},{k:'TARJETA',i:'💳',l:'Tarjeta'}].map(({k,i,l}) => (
            <button key={k} onClick={() => { setMethod(k); setCashGiven(''); }}
              className={`flex flex-col items-center gap-1 p-3 rounded-xl border text-xs font-medium ${method === k ? 'bg-orange-50 border-orange-400 text-orange-600' : 'bg-stone-50 border-stone-200 text-stone-500'}`}>
        ...
        {method === 'EFECTIVO' && (
          <div>
            ...
            <input type="number" className="input" autoFocus value={cashGiven} onChange={e => setCashGiven(e.target.value)} placeholder={`Mínimo $${order.total.toLocaleString('es-CO')}`} />
            {cashGiven && Number(cashGiven) >= order.total && (
              <div className="mt-2 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2 flex justify-between">
                ...
                <span className="text-emerald-700 font-bold"><Currency value={Number(cashGiven) - order.total} /></span>
              </div>
            )}
          </div>
        )}
        ...
        <button onClick={confirm} disabled={!canConfirm || saving} className="btn-success w-full justify-center py-3 font-bold disabled:opacity-40">
...
export default function DeliveryPage() {
  ...
  const load = () =>
    ordersApi.getAll()
      .then(all => setOrders(all.filter(o => o.orderType === 'DOMICILIO' || (o.orderType as string) === 'ONLINE')))
      .catch(console.error);
  ...
  useEffect(() => {
    ...
    return () => { socket.off('new-delivery', load); socket.off('delivery-ready', load); socket.off('order-status-changed', load); };
  }, [socket]);
  ...
  const startSharing = (orderId: number) => {
    ...
    watchRef.current = navigator.geolocation.watchPosition(
      async ({ coords }) => { await api.patch(`/orders/${orderId}/location`, { lat: coords.latitude, lng: coords.longitude }).catch(() => {}); },
  ...
  const payBadge = (o: Order) => o.paymentMethod
    ? <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 border border-emerald-200 font-medium">{o.paymentMethod === 'EFECTIVO' ? '💵' : o.paymentMethod === 'TRANSFERENCIA' ? '📲' : '💳'} Pagado</span>
    : <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-red-50 text-red-600 border border-red-200 font-medium">💰 Por cobrar</span>;

  return (
    <div className="p-4 md:p-6 space-y-6">
      {payOrder && <PaymentModal order={payOrder} onClose={() => setPayOrder(null)} onDone={load} />}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {pending.map(order => {
          ...
          return (
            <div key={order.id} className={`card p-4 space-y-3 ${sysDelivered ? 'border-amber-300' : 'border-orange-200 pulse-delivery'}`}>
              {sysDelivered && (
              ...
              <div className="bg-stone-50 border border-stone-200 rounded-lg p-3 space-y-1">
                {d?.phone        && <p className="text-sm text-stone-600 flex gap-2"><span>📞</span>{d.phone}</p>}
              ...
              <div className="space-y-0.5">
                {order.items.map(item => <p key={item.id} className="text-xs text-stone-500">{item.quantity}× {item.product.name}</p>)}
              </div>
              ...
              {!order.paymentMethod && (
                <button onClick={() => setPayOrder(order)} className="w-full py-2 rounded-lg text-sm font-bold bg-emerald-50 border border-emerald-300 text-emerald-700 hover:bg-emerald-100 transition-colors">
                  💳 Cobrar domicilio
                </button>
              )}

              {order.status === 'PENDING'   && <button onClick={() => handleStatus(order.id, 'PREPARING')} className="w-full py-2.5 rounded-lg text-sm font-bold bg-blue-600 hover:bg-blue-500 text-white">🔵 Marcar preparando</button>}
              {order.status === 'PREPARING' && <button onClick={() => handleStatus(order.id, 'READY')}     className="w-full py-2.5 rounded-lg text-sm font-bold bg-emerald-600 hover:bg-emerald-500 text-white">✅ Marcar listo</button>}
              {order.status === 'READY' && (
                sharingId === order.id
                  ? <button onClick={stopSharing} className="w-full py-2.5 rounded-lg text-sm font-bold bg-red-500 hover:bg-red-400 text-white flex items-center justify-center gap-2"><span className="w-2 h-2 bg-white rounded-full animate-pulse" />Compartiendo — Detener</button>
                  : <button onClick={() => startSharing(order.id)} className="w-full py-2.5 rounded-lg text-sm font-bold bg-blue-600 hover:bg-blue-500 text-white">📍 Compartir ubicación</button>
              )}
              {order.status === 'READY' && (
                <button onClick={() => handleStatus(order.id, 'DELIVERED')} className="w-full py-2.5 rounded-lg text-sm font-bold bg-stone-600 hover:bg-stone-500 text-white">🛵 Marcar en camino</button>
              )}

              <button onClick={() => handleDelivered(order)} className="w-full py-3 rounded-xl text-sm font-bold bg-orange-500 hover:bg-orange-400 text-white shadow-md shadow-orange-200">
                ...
                <tbody>
                  {delivered.map(o => (
                    <tr key={o.id} className="border-b border-stone-100 hover:bg-stone-50">
                      ...
                      <td className="px-4 py-2.5">{payBadge(o)}</td>
